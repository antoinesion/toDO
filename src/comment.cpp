...
void comment_task (std::map<int, Task*>& id_to_ptr, int argc, char* argv []) {
  ...
  if (argc == 2) {
    std::cerr << "\033[1;31m/!\\ error:\033[0;31m no id detected afer 'comment'.";
  }
  else if (argc > 5) {
    std::cerr << "\033[1;31m/!\\ error:\033[0;31m too many arguements after 'comment'.";
  }
  else {
    ...
    if (id_to_ptr.find(id) == id_to_ptr.end()) {
      ...
      std::cerr << "\033[1;31m/!\\ error:\033[0;31m need an action (new/del) after id.";
    } else {
      if (std::strcmp(argv[3], "new") == 0) {
	if (argc == 4) {
	  std::cerr << "\033[1;31m/!\\ error:\033[0;31m no comment detected.";
	} else {
	  std::string cmt (argv[4]);
	  if (cmt.find("\n") != std::string::npos) {
	    std::cerr << "\033[1;31m/!\\ error:\033[0;31m no multi-lined comment allowed.";
	  }
	  else {
	...
	if (argc == 4) {
	  std::cerr << "\033[1;31m/!\\ error:\033[0;31m no comment number detected.";
	} else {
	...
	std::cerr << "\033[1;31m/!\\ error:\033[0;31m action must be 'new' or 'del'.";
      }
    }
  }
};
