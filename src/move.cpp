...
bool move_task (Task* task_to_move, int* where_id, int* positioning,
  ...
  if (argc == 2) {
    std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m no id detected afer 'move'.";
  } else {
    int id = std::stoi(argv[2]);
    if (id_to_ptr.find(id) == id_to_ptr.end()) {
      std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m no match for id '" << argv[2] << "'. ";
    } else if (argc == 3) {
      std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m no positioning argument detected after task id.";
    } else {
      bool error = false;
	...
	*positioning = 1;
	...
	std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m positioning argument must be 'above' or 'under'.";
	error = true;
      }
      if (!error) {
	if (argc == 4) {
	  std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m no id detected afer '" << argv[3] << "'.";
	} else {
	  *where_id = std::stoi(argv[4]);
	  if (id_to_ptr.find(*where_id) == id_to_ptr.end()) {
	    std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m no match for id '" << argv[4] << "'. ";
	  } else if (*where_id == id) {
	    std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m task to move and positioning argument cannot be the same.";
	  } else {
		...
		std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m task (id:" << id << ") and task (id:" << argv[4]
		  << ") aren't under the same task.";
	      }
	    } else {
	      std::cerr << "\033[1;38;5;9m/!\\ error:\033[0;38;5;9m task (id:" << id
